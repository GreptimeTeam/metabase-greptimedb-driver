(ns metabase.driver.greptimedb.actions
  "Method impls for [[metabase.driver.sql-jdbc.actions]] for `:greptimedb`."
  (:require
...
(defmethod sql-jdbc.actions/parse-sql-error :greptimedb
...
(defmethod sql-jdbc.actions/base-type->sql-type-map :greptimedb
  [_driver]
...
(defmethod sql-jdbc.actions/do-nested-transaction :greptimedb
...
(defmethod sql-jdbc.actions/prepare-query* [:greptimedb :row/create]
  [_driver _action hsql-query]
  (assoc hsql-query :returning [:*]))

;;; Result is already the created row.
(defmethod sql-jdbc.actions/select-created-row :greptimedb
